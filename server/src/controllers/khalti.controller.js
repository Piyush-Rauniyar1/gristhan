...
export const initiateKhaltiPayment = async (req, res) => {
  ...
  try {
    ...
    const khaltiPayload = {
      ...
      purchase_order_name: `Booking ${booking.id.slice(0, 8)}`,
    };

    console.log(
      "[KHALTI] Sending v2 Initiate Payload:",
      JSON.stringify(khaltiPayload, null, 2),
    );

    const khaltiResponse = await axios.post(khaltiInitiateUrl, khaltiPayload, {
      headers: {
  ...
  try {
    ...
    if ((!token && !pidx) || (!amount && !pidx) || !booking_id) {
      return res
        ...
        .json({
          success: false,
          message: "Verification parameters and booking_id are required",
        });
    }
    ...
    try {
      khaltiResponse = await axios.post(endpoint, payload, {
        headers: {
          Authorization: `Key ${process.env.KHALTI_SECRET_KEY}`,
          "Content-Type": "application/json",
        },
      ...
      console.error(
        `[KHALTI] API Connection Error (Status ${apiStatus}):`,
        apiData || apiError.message,
      );
    ...
    console.log(
      `[KHALTI] API Response Data:`,
      JSON.stringify(khaltiResponse.data, null, 2),
    );
    ...
    console.log(
      `[KHALTI] Payment CONFIRMED for booking ${booking_id}. Updating local database...`,
    );
    ...
    try {
      ...
      if (bookingInfo.rows.length > 0) {
        ...
        const pb =
          typeof bi.price_breakdown === "string"
            ? JSON.parse(bi.price_breakdown)
            : bi.price_breakdown;
        ...
        await query(
          ...
          [
            bi.guest_id,
            "Booking Confirmed! 🎉",
            `Your stay at ${bi.listing_title} has been confirmed.`,
        ...
        await query(
          ...
          [
            ...
            `${bi.guest_name} just booked ${bi.listing_title}.`,
            "new_booking",
          ],
        );
        ...
        console.log(
          `[KHALTI] Notifications and emails sent for booking ${booking_id}`,
        );
      }
    } catch (notifErr) {
      console.error(
        "[KHALTI] Notification/Email block failed:",
        notifErr.message,
      );
    }
